...
/* @var $model frontend\models\FlexSearch */
 ...
 //   'form' => $form, // best for correct client validation
	...
	if (Yii::$app->user->can('admin')){ ?>
		...
		if (isset($model->date_from)) {
			if (Flex::find()->where(['between', 'date', $model->date_from, $model->date_to])->all()  != null) {  
		
			?>
			
			<?= Html::a(Yii::t('app', 'Отчет'), ['export',
			'aq_fl_id'=>81,'date_from'=>$model->date_from,'date_to'=>$model->date_to,], ['class' => 'btn btn-danger']) ?>
			<?= Html::a(Yii::t('app', 'Документы'), ['documents',
			'aq_fl_id'=>81,'date_from'=>$model->date_from,'date_to'=>$model->date_to], ['class' => 'btn btn-success']) ?>
			<?php	
