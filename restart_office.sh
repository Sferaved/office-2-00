...
if ! curl -s --head --request GET https://korzhov-office.kharkiv.ua/ | head -n 1 | grep -q "200"; then
    echo "$current_time - Сайт недоступен. Перезапускаем контейнер." >> /var/log/restart_office.log
    ...
    echo "$current_time - Сайт доступен. Перезапуск не требуется." >> /var/log/restart_office.log
fi
